...
| 2 | `notebooks/02_select_stocks_clustering_mst.ipynb` | Use Spearman correlation, hierarchical clustering, and excess-Sharpe selection to pick one stock per cluster. |
| 3 | `notebooks/03_allocate_portfolios.ipynb` | Allocate portfolio weights with Equal Weight, Inverse Volatility, Markowitz frontier, Risk Parity, and CVaR with scenarios. |
...
|---|---|---|---|
| Markowitz / Mean-Variance | Efficient-frontier idea and mean-risk tradeoff | MOSEK-style frontier: maximize `annual_return - delta * annual_volatility`; 10 log-spaced delta values over MOSEK's example range | https://docs.mosek.com/portfolio-cookbook/markowitz.html |
...
| Minimum observations for Sharpe selection | `756` trading days | Require about three trading years before a stock can be selected by Sharpe |
...
| Markowitz delta grid | `np.logspace(-1, 1.5, 10)[::-1]` | 10 values over the same range used in the MOSEK example |
| Markowitz headline representative | Max-Sharpe point from Markowitz frontier | Avoid presenting any single delta as theoretically fixed |
...
| Benchmark | `^GSPC` | S&P 500 index benchmark |
...
| Transaction cost | `0.001` | Same convention as the source repo, charged on turnover |
...
| CVaR Monte Carlo scenarios | Multivariate normal scenarios fitted from historical mean/covariance | Used only to optimize CVaR weights |
...
| `outputs/portfolio_markowitz_frontier_summary.csv` | Main Markowitz result: 10 delta points on the efficient frontier |
| `outputs/portfolio_weights_markowitz_best_sharpe_default.csv` | Representative Markowitz point used in the headline comparison |
...
1. The S&P 500 universe comes from the current Wikipedia page, so historical backtests using this universe may have survivorship/constituent bias.
2. Yahoo Finance data availability and ticker mapping can change over time.
3. Markowitz expected returns are estimated from historical returns and can be noisy.
4. Monte Carlo scenarios assume multivariate normal returns, which may understate fat tails.
5. Bootstrap scenarios preserve same-day cross-asset co-movement but do not preserve serial dependence unless block bootstrap is added later.
6. The 10% max-weight cap is a risk-control assumption, not a requirement from MOSEK or the original repo.
7. Step 5 removes look-ahead leakage from stock selection by reselecting stocks at every rebalance with past data only, but it still does not solve point-in-time S&P 500 membership bias.
