...
| 02   | notebooks/02_select_stocks_clustering_mst.ipynb       | Cluster stocks and select one high-Sharpe stock per cluster using latest full history |
| 03   | notebooks/03_allocate_portfolios.ipynb                | Create current portfolio allocations and efficient-frontier diagnostics               |
| 04   | notebooks/04_backtest_allocation_only.ipynb           | Allocation-only walk-forward test on the fixed 25 stocks from Step 02                 |
| 05   | notebooks/05_backtest_full_pipeline_walkforward.ipynb | Full-pipeline walk-forward test with stock selection rerun each rebalance             |
...
| Markowitz          | 1.5341      | 10.41% | 0.4013 | -30.50%      | 19.25%     | 30.61    |
...
- The S&P 500 universe is based on the current Wikipedia constituent list, not point-in-time historical membership. This can create survivorship and constituent bias.
